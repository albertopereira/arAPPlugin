...
class InformationObjectBrowseAction extends DefaultBrowseAction
{
  ...
  public function execute($request)
  {
    ...
    $this->context->user->getAttributeHolder()->remove('denied-ids');

    if (!isset($this->context->user['denied-ids'])) {
      $new_search = new \Elastica\Query();
      $new_search->setQuery(new \Elastica\Query\MatchAll());
      $new_search->setSize(10000);
      $new_search->setFrom(0);
      $new_search->setStoredFields(['id']);

      $resultSet_1 = QubitSearch::getInstance()->index->getType('QubitInformationObject')->search($new_search);

      foreach ($resultSet_1->getResults() as $idx => $hit):
          $a = QubitObject::getById($hit->getId())->getAncestors(); 
          ...
          foreach ($ids as $value) {
            if(!QubitAcl::isAllowed(sfContext::getInstance()->user, (string)$value, 'read')){
              // unset($resultSet_1->_results[$idx]);
              $remove_ids[] = $resultSet_1->getResults()[$idx]->getId();
              // $resultSet_1->_totalHits -= 1;
            }
          }
      endforeach;

      $this->context->user->setAttribute('denied-ids', $remove_ids);

    }

    $idsQuery = new \Elastica\Query\Ids();
    $idsQuery->setIds($this->context->user->getAttribute('denied-ids'));
    $this->search->queryBool->addMustNot($idsQuery);
    // $this->search->queryBool->addMustNot(new \Elastica\Query\Ids('', array($this->context->user->getAttribute('denied-ids'))));
